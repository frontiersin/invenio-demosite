...
class ScopusJsonReader(Reader):
    """Default reader"""

    __master_format__ = 'scopus_json'
    ...
    def split_blob(blob, schema=None, **kwargs):
        ...
        blob = blob['search-results']['entry']
        for b in blob:
            yield json.dumps(b)
    ...
    def _prepare_blob(self):
        ...
        self._blob = json.loads(self._blob)
    ...
    def _unpack_rule(self, json_id, field_name=None):
        super(ScopusJsonReader, self)._unpack_rule(json_id, field_name)
    ...
    def _apply_virtual_rules(self, json_id, field_name, rule):
        ...
        if json_id in self._blob:
            ...
            super(ScopusJsonReader, self)._apply_virtual_rules(json_id, field_name,
                                                         rule)


reader = ScopusJsonReader
